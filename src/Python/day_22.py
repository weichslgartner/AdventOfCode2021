...
def parse_input(lines):
    return [True if line.startswith("on") else False for line in lines], \
           [tuple(map(int, re.findall(r"-?\d+", line))) for line in lines]
...
def solve(on_off, cubes, max50: bool) -> int:
    cube_archive = set()
    n_turned = 0
...
def tests():
    ...
    get_overlap([-48, -32, 26, 41, -47, -37], [-46, -23, 21, 46, -50, -30]) == [-46, -23, 21, 46, -47, -37]


def cubes_to_set(cubes):
    points = set()
    for c in cubes:
        points |= to_points(c)
    return points
